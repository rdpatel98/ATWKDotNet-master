    ...
    public class GenericAuthenticationFilter : AuthorizationFilterAttribute
    {
        ...
        protected virtual BasicAuthenticationIdentity FetchAuthHeader(HttpActionContext filterContext)
        {
            ...
            try
            {
                ...
                //var newText = regex.Replace(authHeaderValue, "ATWKSEP", 1);
                //var credentials = Regex.Split(authHeaderValue, "ATWKSEP"); 
                var credentials = authHeaderValue.Split(':');
                return credentials.Length < 2 ? null : new BasicAuthenticationIdentity(credentials[0], String.Join(":", credentials.Skip(1).ToArray()));
            }
            catch (Exception exp)
            {
                return null;
